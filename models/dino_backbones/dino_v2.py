...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        ffn_layer="mlp",
        block_chunks=0,
        out_indices=[7, 11, 15, 23],   # Large      
        # out_indices=[9, 15, 19, 23],   # Large   
    ...
    def forward_features(self, x, name, masks=None):
        ...
        for idx, blk in enumerate(self.blocks):
            ...
            if idx in self.out_indices:
                x, x_l, x_h = self.freq_decoup.forward(x, idx, batch_first=True, has_cls_token=True)
                outs_l.append(
                    x_l[:, 1:, :] 
                    .permute(0, 2, 1)
