...
class ICB(L.LightningModule):
    def __init__(self, in_features, hidden_features, drop=0.):
        ...
        self.conv = nn.Conv2d(1024, 1024, kernel_size=1, stride=1, 
                               padding=0, bias=False, groups=1024)
...
class Adaptive_Spectral_Block(nn.Module):
    def __init__(self, dim):
        ...
        self.complex_weight_low = nn.Parameter(torch.randn(dim, 2, dtype=torch.float32) * 0.02)
    ...
    def forward(self, x_in):
        ...
        x_weighted = x_fft * weight_high

        if True:
            # Adaptive High Frequency Mask (no need for dimensional adjustments)
            freq_mask = self.create_adaptive_low_freq_mask(x_fft)
            x_masked = x_fft * freq_mask.to(x.device)

            weight_low = torch.view_as_complex(self.complex_weight_low)
            x_weighted2 = x_masked * weight_low
...
class TSLANet_layer(L.LightningModule):
    def __init__(self, dim, mlp_ratio=3., drop=0., drop_path=0., norm_layer=nn.LayerNorm):
        ...
        self.mlp_feat = nn.Linear(1024, 1024)
        # nn.init.kaiming_uniform_(self.mlp_feat, a=math.sqrt(5))
