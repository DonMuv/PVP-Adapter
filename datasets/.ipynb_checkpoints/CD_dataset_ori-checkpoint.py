...
def get_val_label_path(root_dir, img_name):
    return os.path.join(root_dir, ANNOT_FOLDER_NAME, img_name.replace('.jpg', val_label_suffix))
...
class CDDataset(ImageDataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split=='train':
            ...
            L_path = get_val_label_path(self.root_dir, self.img_name_list[index % self.A_size])
